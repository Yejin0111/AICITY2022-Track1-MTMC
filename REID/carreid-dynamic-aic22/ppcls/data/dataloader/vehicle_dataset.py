    ...
    def _load_anno(self):
        ...
        with open(self._cls_path) as fd:
            ...
            for l in lines:
                ...
                l = l.split('/')[-1]
                content = l[:-4]
                content_sp = content.split('_')
                if len(content_sp) > 3:
                    if not self._has_ids:
                        self.labels.append(np.int64(1))
                        self.cameras.append(np.int64(1))
                    else:
                        cur_label = content_sp[0]
                        cur_cam = content_sp[1]
                        if 'c' in cur_cam:
                            cur_cam = cur_cam[1:]
                        if cur_label not in remap_ids.keys():
                            remap_ids[cur_label] = len(remap_ids.keys())
                        self.labels.append(np.int64(remap_ids[cur_label]))
                        self.cameras.append(np.int64(cur_cam))
                else:
                    if not self._has_ids:
                        self.labels.append(np.int64(1))
                        self.cameras.append(np.int64(1))
                        ...
                        self.cameras.append(np.int64(cur_cam))
                assert os.path.exists(self.images[-1])
        cur_num_class = len(set(self.labels))
        print('num_classes: ', cur_num_class)
