...
class Engine(object):
    def __init__(self, config, mode="train"):
        assert mode in ["train", "eval", "infer", "export", "eval_feature"]
        self.mode = mode
        self.config = config
        ...
        assert self.eval_mode in ["classification", "retrieval", "retrieval_feature"], logger.error(
        ...
        if self.mode == "eval" or self.mode == "eval_feature" or (self.mode == "train" and
                                   self.config["Global"]["eval_during_train"]):
            if self.eval_mode == "classification":
                self.eval_dataloader = build_dataloader(
                    self.config["DataLoader"], "Eval", self.device,
                    self.use_dali)
            elif self.eval_mode == "retrieval" or self.eval_mode == "retrieval_feature":
    ...
    @paddle.no_grad()
    def eval_feature(self, epoch_id=0):
        assert self.mode in ["eval_feature"]
        self.model.eval()
        gallery_feas, query_feas = self.eval_func(self, epoch_id)
        #pdb.set_trace()
        self.model.train()
        return gallery_feas, query_feas
